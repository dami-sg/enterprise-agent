...
/** Render a `tool-approval-required` into a chat view (gateway §6.1). Uses light
 *  Markdown (**bold** / `code`) which each adapter renders natively — Telegram as
 *  rich HTML, WeChat as plain text. */
...
/** A compact labelled detail line for a tool input, when no grantScope is given. */
function summarizeInput(input: unknown): string {
  if (input == null || typeof input !== 'object') return '';
  const o = input as Record<string, unknown>;
  const labels: Record<string, string> = { command: '命令', path: '路径', url: '链接', file: '文件' };
