...
var I18N = {
  zh: {
    ...
    sttTitle: 'ASR · 语音转写', sttHint: '配置转写语音消息的后端（OpenAI Whisper / StepFun / 任意 OpenAI 兼容端点）。密钥只写入 keychain，重启网关生效。',
    sttProvider: '提供方', sttOff: '（关闭）', sttCustom: '自定义（OpenAI 兼容）', sttKey: 'API Key',
    sttKeyPh: '输入 API Key', sttKeySaved: '已保存（留空不改）', sttModel: '模型（可空，按提供方默认）',
    sttBase: 'baseURL（仅自定义需要）', sttLang: '语言（可空，如 zh）', sttSave: '保存 ASR 配置',
  ...
  en: {
    ...
    sttTitle: 'ASR · Speech-to-text', sttHint: 'Backend that transcribes voice messages (OpenAI Whisper / StepFun / any OpenAI-compatible endpoint). The key is stored in the keychain only; restart the gateway to apply.',
    sttProvider: 'Provider', sttOff: '(off)', sttCustom: 'Custom (OpenAI-compatible)', sttKey: 'API Key',
    sttKeyPh: 'Enter API key', sttKeySaved: 'Saved (blank = keep)', sttModel: 'Model (optional; provider default)',
    sttBase: 'baseURL (custom only)', sttLang: 'Language (optional, e.g. zh)', sttSave: 'Save ASR config',
