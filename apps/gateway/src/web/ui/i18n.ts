...
var I18N = {
  zh: {
    ...
    mdDeclare: '自动检测不准时（如自定义/新发布的多模态模型，目录里查不到），在此手动声明模型支持的模态，透传即可生效：',
    mdAuto: 'auto（支持视觉则透传，否则存盘）', mdPassthrough: 'passthrough 透传', mdOff: 'off 忽略', mdAgent: 'agent 存盘（让 agent 解析）', mdSave: '保存媒体配置',
  ...
  en: {
    ...
    mdDeclare: 'When auto-detection is wrong (a custom / newly-released multimodal model the catalog can\'t confirm), declare the modalities the model supports so passthrough takes effect:',
    mdAuto: 'auto (passthrough if vision, else save)', mdPassthrough: 'passthrough', mdOff: 'off', mdAgent: 'agent (save for parsing)', mdSave: 'Save media config',
