...
var I18N = {
  zh: {
    ...
    mdCaps: '当前模型支持：', mdImage: '图片', mdPdf: 'PDF', mdAudioWord: '音频',
    mdDeclare: '自动检测不准时（如自定义/新发布的多模态模型，目录里查不到），勾选声明模型支持的模态。注意：声明只是“告诉系统模型能读”，还要把上面的策略选为 auto 或 passthrough，PDF 才会直传给模型（默认 agent 是存盘交给工具解析）。改完需重启网关。',
  ...
  en: {
    ...
    mdCaps: 'Current model supports:', mdImage: 'Image', mdPdf: 'PDF', mdAudioWord: 'Audio',
    mdDeclare: 'When auto-detection is wrong (a custom / newly-released multimodal model the catalog can\'t confirm), tick the modalities the model supports. Note: declaring only tells the system the model can read it — you must also set the strategy above to auto or passthrough for PDFs to reach the model directly (the default agent saves them for tool parsing). Restart the gateway after changes.',
