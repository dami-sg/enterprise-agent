...
| 维度 | 设计 |
| --- | --- |
| 同一运行时 | OpenTUI/Solid 与 `@enterprise-agent/agent`（Node/TS，Bun 兼容）都跑在 Bun 上、同一 JS 运行时。host 用 `createAgentHost()`（[packages/agent/src/index.ts:440](../packages/agent/src/index.ts)）构造，**就是 `ea` 进程里的一个普通对象**。|
...
| 与桌面端对称 | 两个壳同一招式：**同进程嵌 host**。桌面端嵌在 Electron main、渲染到 renderer（经 IPC）；CLI 嵌在 `ea`（Bun）进程、渲染到终端（直接调用）。核心两边都是 `@enterprise-agent/agent`，零分叉。|
...
const host = createAgentHost({
  ...
  keychain: osKeyStore,                     // CLI 提供 OS keychain 后端（§7）
});
```
...
| `ea auth <login\|logout\|ls>` | 配置/更新 Provider 密钥（§7，cli-ui §10） | `KeyStore`（写 keychain）|
...
| `ea agent <ls\|show\|add>` | 子 Agent 定义（声明式，生效作用域，agent §2.3，cli-ui §9.4.1） | `AgentRegistry`（导出）+ `ea config agents`/`delegate` |
...
const host = createAgentHost({ root, keychain: osKeyStore });
...
- daemon 模式（§8）下 TUI 重连才需要 `Last-Event-ID` 补播；进程内嵌不涉及。
...
| 关注点 | 设计 |
| --- | --- |
| Keychain 后端 | host 的 `AgentHostOptions.keychain` 默认是 `EnvKeyStore`（读环境变量，[index.ts:73](../packages/agent/src/index.ts)）。CLI **应提供 OS keychain 后端**——实现 `KeyStore` 接口包 macOS `security` / Linux libsecret / Windows credential store（agent §4），`ea auth login` 收 key → `keychain.set()`，配置只留 `keyRef`。|
...
```bash
ea serve --detach            # 显式起 daemon：同一个 createAgentHost()，外面架 HTTP+SSE
ea --server http://127.0.0.1:4096   # TUI 改连 server，而非本进程嵌 host
ea run --server <url> -p "..."      # headless 也连 server
```

- daemon = **同一个 host + 一层传输**：命令走 HTTP（请求式），事件走 SSE（`host.onEvent` fan-out 给所有连接）。形状与 agent §6「请求式命令 + 单向事件流」同构（对照见附录 A）。
- **安全全在 host 侧执行**：文件边界、沙箱、审批、密钥解析都在 server 进程（agent §4），客户端只是远程 I/O——远程跑和本地跑安全模型一致。
- 远程审批：`tool-approval-required` 经 SSE 流回客户端 TUI，用户本地按键、决策经 HTTP 回 server。

### 8.2 与进程内嵌的关系

daemon **不是另一套架构**，只是把 §1 那个 host「挪到独立进程 + 加传输」。OpenTUI TUI 的渲染层、reduceTrace、按键映射**完全不变**——区别只在「`host` 是本地对象，还是一个把方法转成 HTTP 调用的代理」。所以从进程内嵌升级到 daemon，TUI 代码近乎零改动。
...
### A.2 daemon 模式（可选）——HTTP + SSE

把 A.1 的直接调用换成传输；这是桌面端 Electron IPC（desktop §1.2）的 CLI 对位物：

| 方法 | HTTP |
| --- | --- |
| `startSession` | `POST /sessions` → `{ sessionId, runId }` |
| `sendMessage` | `POST /sessions/:id/message` → `{ runId }` |
| `approveTool` | `POST /tool-approvals/:toolCallId` body=`{ decision }` |
| `abortRun` | `POST /runs/:runId/abort` |
| `getSessionTree` | `GET /sessions/:id/tree` → `SessionTree` |
| `forkFrom` / `labelEntry` / `compact` / `cloneToSession` / `getTodos` / `report` | `POST\|GET /sessions/:id/...` |
| Session 管理（list/create/switch/delete/config） | `GET\|POST\|PATCH\|DELETE /sessions` |
| `onEvent` | `GET /events`（SSE，`AgentStreamEvent` 流，`Last-Event-ID` 补播）|

**注意事项**

- 契约的**类型即 wire schema**：请求/响应体直接是 `@enterprise-agent/agent-contract` 的导出类型（`CreateSessionInput`、`ScopedConfig`、`sessionId`、`AgentStreamEvent`…），可生成 OpenAPI / 客户端 SDK，客户端零手写 DTO。
- A.1 与 A.2 共用 §5.3 的 `reduceTrace` 与组件层：进程内嵌时 `host` 是本地对象，daemon 时 `host` 是 HTTP 代理，TUI 之上无感。
- 升级核心包后若 §6 契约增删：进程内嵌走 TS 类型检查即暴露；daemon 模式重生成 SDK，CLI 业务码不动。
