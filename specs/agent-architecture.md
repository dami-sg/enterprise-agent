...
2. **根目录单一来源**：`buildLocalTools(ctx)` 里的文件工具一律以 `ctx.rootPath` 为根做规范化校验。`rootPath` 由「Session 指定的 `workingDir`，否则默认工作目录」唯一确定（§1.1）。
...
- **策略来源**：`allowWrite =` Session 工作目录（指定 `workingDir` → 该目录；否则 → 私有 `scratch/`，§1.1）、`network` 取生效网络白名单 —— 把 §1/§4 的边界从「JS 校验」下沉到内核强制。
...
├─ settings.json                      # 全局默认：model 默认、sandbox.enabled、权限策略、compactRatio(默认 0.9)、defaultWorkingDir...
...
└─ sessions/<session-id>/             # 唯一会话实体（§1），扁平、各自独立配置
   ├─ session.json                    # { name, workingDir?, config:{model,sandbox,permission,aliases 覆盖}, headEntryId, todos, usage }
