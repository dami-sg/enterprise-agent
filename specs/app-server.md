...
| Web 接入 | ~~`apps/web` + `/api/chat` SSE~~ **已移除**（gateway-consolidation §P4）；所有富客户端统一走 `WS /rpc` + `@enterprise-agent/agent-client` |
...
| Web integration | ~~`apps/web` + `/api/chat`~~ **已移除**（§P4）。富客户端统一经 `WS /rpc` + `@enterprise-agent/agent-client`；浏览器用 cookie，桌面/移动用 Bearer access key。 |
