...
> 状态：MVP 已落地。本文定义 **Enterprise Agent App Server**：供 Web、桌面端、移动端、IDE 插件等富客户端共享的一层统一协议服务。它不重写 agent runtime，而是把现有 `@enterprise-agent/agent` 的 `AgentHost` 命令/事件契约包装成可远程连接、可鉴权、可重连的 client/server 接口。
>
...
| Web 接入 | `apps/web` 默认保留 `/api/chat` SSE；URL 加 `?rpc` 使用 `@enterprise-agent/agent-client` 连接 `/rpc` |
...
| 范围 | 状态 |
| --- | --- |
| Spec + packages | 已新增本文档、`packages/agent-server`、`packages/agent-client`。`agent-server` 根导出协议/核心，Node listener 通过 `@enterprise-agent/agent-server/node` 子路径导出，避免浏览器包误打入 `node:http`。 |
...
| Gateway integration | 已新增 `startGatewayAppRpcServer(...)` 和 `ea-gateway app-server`，入口为 `/rpc`，健康检查为 `/healthz` / `/readyz`。现有 `/api/chat` SSE 继续保留。 |
| Web integration | `apps/web` 已接入 `@enterprise-agent/agent-client`。默认仍走 `/api/chat`；加 `?rpc` 时切到 `RpcChatView`，通过 same-origin `/rpc` 连接 app-server。Vite dev proxy 已代理 `/rpc` 到 `localhost:7320`。 |
