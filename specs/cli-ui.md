...
│     └ +38 −12                                     ← diff 统计
│
├─ ▾ Sub#researcher  「查 OAuth 最佳实践」            ← 子 Agent 节点（sub-agent-start）
│   │  ● 运行中…                                    ← 嵌套缩进，自己的轨迹
│   ├─ ↗ httpFetch  api.github.com           ✓
│   └─ ✓ 完成 · 返回 3 条建议                        ← sub-agent-finish
│
...
| 节点 | 事件 | 折叠默认 |
| --- | --- | --- |
| Agent（orchestrator / sub） | `sub-agent-start` / `-finish` | 子 Agent 展开、完成后可折叠 |
...
```
 ...
 sandbox         ✓ 启用
 …
 嵌套委派        ✗researcher ✗coder ✗analyst ✗writer   ← 哪些子 Agent role 可再 spawn 子 Agent（agent §2.3 pt.2）
```
...
| 操作 | 键 | 行为 |
| --- | --- | --- |
| 中断当前 run | `Ctrl-C`（模型输出中）| run 在跑时：`abortRun(runId)`（agent §6.3），**只中断不退出** |
| 退出确认 | `Ctrl-C`（空闲时）| 弹出居中确认弹窗「退出 Enterprise Agent？ [y] 确认退出 · 其他键取消」；按 `y` 退出，任意其他键取消 |
...
> `Ctrl-C` 兼顾两义：**模型在输出时**（`trace.status === "running"` 且有 `runId`）是「中断运行」（终端通用语义，只停这次调用）；**空闲时**是「退出」，弹出确认弹窗，`y` 确认、其他键取消，避免误按一次就丢会话。确认弹窗打开时输入框 blur、按键走全局 handler 并 `preventDefault`（不漏进输入框）。
